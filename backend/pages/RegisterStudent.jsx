...
export default function RegisterStudent() {
  const [form, setForm] = useState({ name: "", reg_no: "", class: "", section: "", email: "", password: "" });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white shadow p-6 rounded w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">Student Register (Team Leader)</h2>
        <form onSubmit={submit} className="space-y-3">
          ...
          <input name="class" placeholder="Class" value={form.class} onChange={handleChange} className="w-full border p-2 rounded" />
          <input name="section" placeholder="Section" value={form.section} onChange={handleChange} className="w-full border p-2 rounded" />
          <input required type="email" name="email" placeholder="Email" value={form.email} onChange={handleChange} className="w-full border p-2 rounded" />
