    ...
    <div className="min-h-screen">
      <header className="bg-[#003366] text-white p-4 flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <Link to={`/team/${team?._id}/add-member`} className="bg-yellow-400 px-3 py-2 rounded text-black">Add Member</Link>
          <Link to={`/project/${team?._id}/upload`} className="bg-white text-black px-3 py-2 rounded">Project Upload</Link>
      ...
      <div className="p-6">
        ...
        <div className="bg-white shadow p-4 rounded">
          <p><strong>Leader:</strong> {team?.leader_id?.name || "Not assigned"}</p>
          <p className="mt-2"><strong>Members:</strong></p>
          <ul className="list-disc ml-6">
            { (team?.members?.length > 0) ? team.members.map((m, i) => <li key={i}>{m.name} — {m.reg_no} — {m.email}</li>) : <li>No members added</li> }
          </ul>
          <div className="mt-4">
            <Link to="/faculties" className="text-blue-600">View Faculties / Select Guide Preferences</Link>
          </div>
        </div>
      </div>
    </div>
